 ...
 * is lowered below its nominal tier top. Creates the ACNH-style sand step
 * between grass (Y=0) and the water plane. 0.30 m gives the slope enough
 * vertical drop to read as a real beach descent (vs the previous 0.15 m
 * which made each per-cell slope quad look like a small triangular bump).
 * Requires the water plane Y to sit BELOW this (currently -0.40 in
 * createIslandScene) so dry sand stays above water.
 */
export const BEACH_LOWER_OFFSET_METERS = 0.30;

/**
 * Chebyshev radius (in cells) within which a LAND-T0 cell is considered a beach
 * cell. 3 → ~3 m visible sand band, matching the ACNH coast width at the camera
 * angles used in this game. Decreasing tightens the sand to a sliver; increasing
 * eats into the inland grass and changes the island's silhouette feel.
 */
export const BEACH_RADIUS_CELLS = 3;
...
export class TerrainGrid {
   ...
   *
   * LAND-T0 cells flagged as "beach" (`isBeachCell`) are lowered to
   * -BEACH_LOWER_OFFSET_METERS, producing the ACNH sand-step silhouette at
   * the island border. The slope wall between a normal LAND cell and an
   * adjacent beach cell is auto-generated by `forEachTierDiscontinuity`.
   */
  cellHeight(cx: number, cz: number): number {
    ...
    switch (cell.surface) {
      ...
      case Surface.OCEAN:
        return 0;
      case Surface.LAND:
        if (this.isBeachCell(cx, cz)) return -BEACH_LOWER_OFFSET_METERS;
  ...
  /**
   * Beach detection: a LAND-T0 cell within `BEACH_RADIUS_CELLS` (Chebyshev
   * distance) of an OCEAN cell. Such cells are visually lowered (see
   * `cellHeight`) and rendered with the sand texture (see
   * `surfaceClassification`), creating the ACNH grass→sand step at the island
   * border. Pure function over the grid — recomputed on demand, no separate
   * state to keep in sync.
   */
  isBeachCell(cx: number, cz: number): boolean {
    if (!this.cellInBounds(cx, cz)) return false;
    if (this.getSurface(cx, cz) !== Surface.LAND) return false;
    if (this.getTier(cx, cz) !== Tier.T0) return false;
    for (let dz = -BEACH_RADIUS_CELLS; dz <= BEACH_RADIUS_CELLS; dz += 1) {
      for (let dx = -BEACH_RADIUS_CELLS; dx <= BEACH_RADIUS_CELLS; dx += 1) {
        if (dx === 0 && dz === 0) continue;
        const nx = cx + dx;
        const nz = cz + dz;
        // Out-of-bounds is treated as OCEAN: the grid extents (94×78) are
        // tight against the island silhouette, so the easternmost / westernmost
        // LAND cells have no in-grid OCEAN neighbor — the open sea visually
        // sits past the grid edge on the water plane. Without this rule,
        // boundary LAND cells would never qualify as beach and the visible
        // sand band would have gaps at the four extremes of the island.
        if (!this.cellInBounds(nx, nz)) return true;
        if (this.getSurface(nx, nz) === Surface.OCEAN) return true;
      }
    }
    return false;
  }
